...
function executeCommand() {
  return new Promise((resolve, reject) => {
    console.log("===>executing fog command")
...
function executeCloudCommand() {
  return new Promise((resolve, reject) => {
    console.log("===>executing cloud command")
      exec(`cd present && conda activate torch && python ./demo.py --filename ${newName} --test_filepath ../${process.env.SAVEPATH}/${newName} --pretrained ./pretrained_models/RICE2/r2_200.pth --cuda `, async (error, stdout, stderr) => {
          if (error) {
              console.error('Error executing command:', error);
              reject(false);
              return;
...
const u = await User.findOne({original_image:buffer,name, email, cipherName:newName, gen_image:gen_image_save, method});

if (!u) {
    const newUser = new User({ original_image: buffer, name, email, cipherName: newName ,gen_image,method});
    await newUser.save();
